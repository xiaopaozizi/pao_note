      ...
      return {
        ...
        defCol:[
         // {name: '账单ID', record: "monthlyStatementId"},
          {name: '账单号',  record: "monthlyStatementNo"},
          {name: "收付", record: "receiptPaymentType"},
          {name: "结算方简称",record: "settleAccountsShortName"},
          {name: "结算方全称", record: "settleAccountsFullName"},
          {name: "合计金额",record: "billMoney"},
          {name: "对账人",record: ""},
          {name: '制单日期', record: "createDate"},
          {name: "账单备注", record: "billRemark"},
        ],
        //查询搜索表单
        searchForm: {
          dateStr:'', //传给后台的时间日期
    ...
    methods: {
      ...
      initData(xdtList){
        ...
        xdtList.forEach(function(node, index){
           if(node.billMoney  < 0 ) {
             self.totalPay += node.billMoney;
           } else if(node.billMoney  > 0) {
             self.totalCollect += node.billMoney;

           }else  {

           }
            console.log(node.billMoney);
      ...
      editBtn() {
        if(!this.selectRowData) {
          ...
          let params = {
            relMonthlyStatementId: this.selectRowData.monthlyStatementId,
          };
